...
namespace SistemaBancario2._0.Pages
{
    public class TransaccionesModel : PageModel
    {
        ...
        {
            // Usar la instancia est�tica compartida para que usuarios registrados permanezcan.
        ...
        {
            // Intentar obtener el n�mero de cuenta desde TempData sin consumirlo
            var numeroCuenta = TempData.Peek("NumeroCuenta") as string;

            // Si no est� en TempData, intentar tambi�n leerlo desde Query (opcional)
            ...
            // Mantener el TempData para que siga disponible en la siguiente petici�n
        ...
        public IActionResult OnPostConsignar(decimal monto)
        {
            ...
            try
            {
                UsuarioActual.CuentaBancaria.ConsignarDirecto("Consignaci�n en cajero", monto);
                MensajeExito = $"Consignaci�n exitosa de ${monto:N0}. Nuevo saldo: ${UsuarioActual.CuentaBancaria.Saldo:N0}";
            }
            catch (Exception ex)
            {
            ...
            // Mantener TempData para la pr�xima vista
            TempData.Keep("NumeroCuenta");
            TempData.Keep("NombreUsuario");
        ...
        public IActionResult OnPostRetirar(decimal monto)
        {
            ...
            try
            {
                ...
                else if (UsuarioActual.CuentaBancaria is CuentaCorriente cuentaCorriente)
                {
                    ...
                    {
                        MensajeExito = $"Retiro exitoso de ${monto:N0}. Se utiliz� sobregiro. Saldo: ${UsuarioActual.CuentaBancaria.Saldo:N0}";
                    }
                    else
                    {
        ...
        public IActionResult OnPostTransferir(string cuentaDestino, decimal monto)
        {
            ...
            {
                MensajeError = "El n�mero de cuenta debe tener 5 d�gitos.";
                return RedirectToPage();
            }
        ...
        public IActionResult OnPostCompraCredito(string descripcion, decimal monto, int cuotas)
        {
            ...
            {
                MensajeError = "Esta operaci�n solo est� disponible para tarjetas de cr�dito.";
                return RedirectToPage();
            }

            if (string.IsNullOrEmpty(descripcion))
            {
                MensajeError = "Debes ingresar una descripci�n para la compra.";
                return RedirectToPage();
            }
            ...
            if (cuotas <= 0)
            {
                MensajeError = "El n�mero de cuotas debe ser mayor a cero.";
                return RedirectToPage();
            }
        ...
        // Handler: Pagar Tarjeta de Cr�dito
        public IActionResult OnPostPagarCredito(decimal monto)
        {
            ...
            try
            {
                ...
                MensajeExito = $"Pago realizado de ${monto:N0}. Deuda restante: ${tarjeta.DeudaTotal:N0}. Cr�dito disponible: ${tarjeta.CreditoDisponible:N0}";
            }
            catch (Exception ex)
            {
        ...
        // M�todo auxiliar para cargar el usuario actual
        private void CargarUsuarioActual()
        {
            // Intentar obtener n�mero de cuenta desde TempData sin consumirlo
            var numeroCuenta = TempData.Peek("NumeroCuenta") as string;
            ...
            {
                UsuarioActual = _banco.BuscarUsuarioPorCuenta(numeroCuenta);
                // Mantener TempData para pr�ximas requests
                TempData.Keep("NumeroCuenta");
                TempData.Keep("NombreUsuario");
            }
        }

        // M�todo para obtener el nombre del tipo de cuenta
